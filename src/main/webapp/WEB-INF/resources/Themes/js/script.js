...
$(document).ready(function() {
    ...
    $(".card-header-right .card-option .open-card-option").on('click', function() {
        ...
        if ($this.hasClass('fa-times')) {
            ...
            $(this).addClass("fa-expand").fadeIn('slow');
        } else {
            $this.parents('.card-option').animate({
                'width': '140px',
            });
            $(this).addClass("fa-times").fadeIn('slow');
            $(this).removeClass("fa-expand").fadeIn('slow');
        }
    });
...
var avata = getCookie("AVATA");
document.getElementById("loginUser").innerHTML=name.replace(/\_/g,' ');
document.getElementById("avata_login").src =avata;
});
